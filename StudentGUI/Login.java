 ...
 * @day 2019-12-11
 * 
 * @brief 이 클래스는 실행 시, 소켓 통신을 통해 서버에 로그인 정보를 보내면서 로그인 요청을 한 후, 유저 타입에 따라 학생 메뉴, 관리자 메뉴를 호출한다.
 */
...
public class Login extends JFrame {
	...
	public Login() {
		...
		this.addWindowListener(new MyWindowListener(socket, writer)); // 창 졸료 시 서버 접속 끊기 
		...
		btnOk.addActionListener(new ActionListener() {
			...
			@Override				
			public void actionPerformed(ActionEvent e) { // 로그인 버튼 클릭 시, 서버와 통신하여 로그인 정보를 넘긴 후, 결과 값에 따라 학생 메뉴 또는 관리자 메뉴를 호출한다.
				int userType = 0;
				try
				{
