//입사선발자 납부상태 갱신
//관리자가 사적으로 입사선발자들의 납부여부를 알 수 있다는 가정하에 납부상태를 갱신시킬 수 있다.
...
public class Pay_State_Check extends JFrame {
	...
	public Pay_State_Check(Protocol p_t, ObjectOutputStream oos, ObjectInputStream ois,Socket sk) {
		...
		btnNewButton = new JButton("확인");		//텍스트필드에 적은 학번값을 서버측에 전송하여 납부상태를 갱신시킨다.
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(textField.getText() != null && textField.getText().length() == 8)	//학번이 NULL이거나 8자리가 아닌경우 패킷전송을 하지 않는다.
				{
					try {
					...
					if (p.getSubType() == 2) {		//전송한 학번이 납부대상인지 체크한 뒤 납부상태 갱신에 대한 결과 수신
						if(p.getCode() == 1)
						{
